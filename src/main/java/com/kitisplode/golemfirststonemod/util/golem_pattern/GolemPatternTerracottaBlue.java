...
public class GolemPatternTerracottaBlue extends AbstractGolemPattern
{
	private static final Predicate<BlockState> buildingBlockPredicate = state -> state != null
		&& (state.isOf(Blocks.BLUE_TERRACOTTA));

	public GolemPatternTerracottaBlue(Predicate<BlockState> pPredicate)
	{
		super(pPredicate);
	...
	protected ArrayList<Entity> SpawnGolemForReal(World pLevel, BlockPattern.Result pPatternMatch, BlockPos pPos)
	{
		...
		for (int i = 0; i < 3; i++)
		{
			...
			if (golem != null)
			{
				...
				golem.setPawnType(EntityPawn.PAWN_TYPES.PIK_BLUE.ordinal());
				golems.add(golem);
			}
		}
		return golems;
